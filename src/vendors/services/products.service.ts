...
import { VendorProducts, VendorBranches, Vendors } from '../entities/vendors.entity';
...
export class ProductsService{
    ...
    private search_columns = ["product_code", "name", "description", "product_type"];

    constructor(
        private readonly common : CommonQueries, 
        @Inject('VENDOR_REPOSITORY') private readonly VENDOR_REPOSITORY : Repository<Vendors>,
        @Inject('VENDOR_PRODUCT_REPOSITORY') private readonly PRODUCT_REPOSITORY : Repository<VendorProducts>,
        @Inject('VENDOR_BRANCH_REPOSITORY') private readonly BRANCHES_REPOSITORY : Repository<VendorBranches>){
    }
    ...
    async getProductsByVendorId(vendorId : number, options : searchDto){
        ...
        const rows : Array<VendorProducts> = await query.
        limit(options.limit)
    ...
    async getProducts(options : searchDto, conditions ?: Array<Function>){
    ...
    async updateVendorProduct(productId: number, revisions : updateVendorProductDto){
    ...
    async createVendorProduct(product : newVendorProduct){
        ...
        const _product = {
            ...
            name : product.name,
            product_code : voucherCodeGenerator.generate({ length : 5, count : 1, pattern : `#####`, prefix : `VENDOR-` })[0],
